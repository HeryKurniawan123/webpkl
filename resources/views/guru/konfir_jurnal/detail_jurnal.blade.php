<div class="journal-view">
    <div class="row mb-4">
        <div class="col-md-6">
            ...
            <p class="fw-semibold">{{ \Carbon\Carbon::parse($journal->tgl)->locale('id')->isoFormat('dddd, D MMMM YYYY') }}</p>
        </div>
        <div class="col-md-6">
            <h6 class="text-muted">Waktu Kegiatan</h6>
            <p class="fw-semibold">{{ $journal->jam_mulai }} - {{ $journal->jam_selesai }}</p>
        </div>
    </div>
    ...
    <div class="mb-4">
        ...
        <div class="p-3 bg-light rounded">
            <p class="mb-0" style="line-height: 1.6;">{!! nl2br(e($journal->uraian)) !!}</p>
        </div>
    </div>
            ...
            @elseif($journal->status == 'approved')
                ...
                @if($journal->approved_by)
                    <div class="mt-2 small text-success">
                        <i class="bi bi-check-circle"></i> Disetujui oleh: {{ $journal->approved_by }}
                    </div>
                @endif
            @else
