<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <title>Surat Balasan PKL</title>
    <style>
        body {
            font-family: Arial, sans-serif;
            margin: 40px;
            padding: 0;
            color: #333;
        }
        .header {
            text-align: center;
            margin-bottom: 30px;
        }
        .header h2 {
            margin: 0;
            font-size: 20px;
            text-decoration: underline;
        }
        .content {
            line-height: 1.8;
            font-size: 14px;
            text-align: justify; /* teks paragraf jadi rata kanan kiri */
        }
        .content p {
            margin: 8px 0;
        }
        .info-table {
            width: 100%;
            margin-bottom: 20px;
        }
        .info-table td {
            padding: 4px 8px;
            vertical-align: top;
        }
        .info-table .label {
            width: 220px;
        }
        /* .table-data {
            width: 100%;
            border-collapse: collapse;
            margin: 20px 0;
        }
        .table-data th, .table-data td {
            border: 1px solid #ddd;
            padding: 10px;
            text-align: center;
        }
        .table-data th {
            background-color: #f9f9f9;
        } */
        .footer {
            margin-top: 50px;
            text-align: right;
            font-size: 14px;
        }
        .footer p {
            margin: 4px 0;
        }
    </style>
</head>
<body>

    <div class="header">
        <h2>SURAT PERNYATAAN</h2>
    </div>

    <div class="content">
        <p>Yang bertandatangan di bawah ini:</p>

        <table class="info-table">
            <tr>
                <td class="label">Nama Lengkap</td>
                <td>:</td>
                <td>....................................................</td>
            </tr>
            <tr>
                <td class="label">Perusahaan / Institusi</td>
                <td>:</td>
                <td>....................................................</td>
            </tr>
            <tr>
                <td class="label">Alamat Perusahaan / Institusi</td>
                <td>:</td>
                <td>....................................................</td>
            </tr>
            <tr>
                <td class="label">Contact Person</td>
                <td>:</td>
                <td>....................................................</td>
            </tr>
        </table>

        <p>
            Dengan ini menyatakan bahwa kami 
            <b>(Bersedia / Tidak Bersedia)</b> menerima siswa / siswi SMK Negeri 1 Kawali untuk 
            melaksanakan Praktik Kerja Lapangan (PKL) dari tanggal 
            <strong>13 Oktober 2025</strong> sampai dengan tanggal 
            <strong>14 Februari 2026</strong> dengan jumlah siswa ...... orang.
        </p>

        <p>Nama - nama siswa / siswi tersebut adalah sebagai berikut :</p>

        <table border="1" cellspacing="0" cellpadding="5" style="border-collapse: collapse; width: 100%; font-size: 14px;">
            <thead>
                <tr>
                    <th style="text-align: center; width:3%;">NO</th>
                    <th style="text-align: center; width:30%;">NAMA</th>
                    <th style="text-align: center; width:20%;">KELAS</th>
                    <th style="text-align: center; width:17%;">NIS</th>
                    <th style="text-align: center; width:18%;">TEMPAT</th>
                    <th style="text-align: center; width:12%;">KET</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>1</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>
            </tbody>
        </table>

        <p>
            Diharapkan siswa / siswi bersangkutan dapat melaksanakan kegiatan sesuai peraturan dan ketentuan yang berlaku di Perusahaan / Institusi kami.
        </p>

        <p>
            Demikian surat pernyataan ini dibuat untuk dipergunakan sebagaimanamestinya.
        </p>
    </div>

    <div class="footer">
        <table style="width: 100%;">
          <tr>
            <td style="width: 76%;"></td> <!-- Kosongkan separuh kiri -->
            <td style="text-align: center;">
              <p>Ciamis, 28 April 2025</p>
              <br><br><br>
              <p>....................</p>
            </td>
          </tr>
        </table>
      </div>

</body>
</html>
